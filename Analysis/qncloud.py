...
qncloud = wrf.getvar(nc, 'QNCLOUD', timeidx=32)
...
# data = wrf.to_np(qnwfa[0,:,:])
data = wrf.to_np(qncloud[16,:,:])
...
cbar.set_label(qncloud.units)

# Set the map limits.  Not really necessary, but used for demonstration.
ax.set_xlim(wrf.cartopy_xlim(qncloud))
ax.set_ylim(wrf.cartopy_ylim(qncloud))

# Add the gridlines
ax.gridlines(color="black", linestyle="dotted")

plt.title(qncloud.description+'\n'+str(qncloud.Time.values))
